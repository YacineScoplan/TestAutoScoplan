...
Valider et envoyer le doc
    Sleep    10s
    Wait Until Page Contains Element    //button[@class="btn btn-success confirmCoords"]
    Click Element    //button[@class="btn btn-success confirmCoords"]
    Sleep    10s
    Wait Until Page Contains Element    //button[@class="btn btn-success confirmCoords"]
    Click Element    //button[@class="btn btn-success confirmCoords"]
    Sleep    10s
...
Verification de la reception du doc dans le compte CMI (FDP): Partie "Docs"
    Sleep    10s
    Wait Until Page Contains Element    xpath=(//a[@class="baas tesss"])[3]
    Click Link    xpath=(//a[@class="baas tesss"])[3]
    Sleep    10s
    Page Should Contain    FACTURE 1.pdf
    Page Should Contain    FACTURE 2.pdf
    Page Should Contain    FACTURE 3.pdf
...
Verification de la reception du doc dans le compte particulier: "Accueil"
    ...
    Sleep    10s
    Capture Page Screenshot    ${CURDIR}${/}Capture Yousign${/}CaptureTSE4_Accueil.png
    Close Browser
...
Verification de la reception du doc dans le compte particulier: Onglet "Docs"
    ...
    Sleep    10s
    Capture Page Screenshot    ${CURDIR}${/}Capture Yousign${/}CaptureTSE4_OngletDoc.png
    Sleep    10s
    Close Browser
...
Verification de la SE dans le mail
    ...
    Sleep    3s
    Capture Page Screenshot    ${CURDIR}${/}Capture Yousign${/}CaptureTSE4_MAIL.png
